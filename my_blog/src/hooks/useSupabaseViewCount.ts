    ...
    async function run() {
      ...
      if (coolDownMinutes && coolDownMinutes > 0) {
        try {
          ...
          document.cookie = `${cookieKey}=${encodeURIComponent(String(next))}; expires=${expires}; path=/; SameSite=Lax`
        } catch (_) {}
      }
      ...
      try {
        ...
        const { data, error } = await supabase.rpc('inc_post_view', { p_slug: slug })
