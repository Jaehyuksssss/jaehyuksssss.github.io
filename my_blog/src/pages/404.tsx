...
const StyledSVG = styled.svg`
  ...
  #number-0 {
    animation: ${appearFromBottom} 3s cubic-bezier(0.79, 1.31, 0.82, -0.36);
  }

  #alien_hand-R,
  #alien_hand-L {
    animation: ${shakingUpAndDown} 1s infinite cubic-bezier(0.35, 0.28, 0.82, 0.87);
  }
...
const NotFoundPage: FunctionComponent = function () {
  ...
  useEffect(() => {
    const setZeroAndBeamAppear = () => {
      const number0Soaking = svgRef.current?.getElementById('number-0-soaking')
      const ufoBeamDisappearing = svgRef.current?.getElementById('UFO-beam-disappearing')
      
      if (number0Soaking && ufoBeamDisappearing) {
        ...
        const number0 = svgRef.current?.getElementById('number-0')
        if (number0) {
    ...
    const setZeroAndBeamDisappear = () => {
      const number0 = svgRef.current?.getElementById('number-0')
      const ufoBeam = svgRef.current?.getElementById('UFO-beam')
      ...
      if (number0 && ufoBeam) {
        ...
        const number0Soaking = svgRef.current?.getElementById('number-0-soaking')
        if (number0Soaking) {
          number0Soaking.addEventListener('animationend', setZeroAndBeamAppear)
        }
      }
    }
